...
export async function cachePublicMiddleware(c: BaseContext, next: any) {
  c.res.headers.set("Cache-Tag", "static");
  c.res.headers.set("Cache-Control", "public, max-age=600, stale-while-revalidate=3600");
  await next();
};

export async function cacheStaticFileMiddleware(c: BaseContext, next: any) {
...
export async function cachePrivateMiddleware(c: BaseContext, next: any) {
