...
export function ScheduledPost(props: ScheduledPostOptions) {
  ...
  const oobSwapStr = (props.dynamic) ? `hx-swap-oob="#${content.postid}"` : "";
  ...
  return html`
  <article id="${content.postid}" ${oobSwapStr}>
    <div id="post${content.postid}">
      ${hasBeenPosted ? '' : raw(`<button type="submit" hx-delete="/post/delete/${content.postid}" hx-confirm="Are you sure you want to delete this post?" hx-target="#posts" hx-swap="innerHTML" hx-trigger="click" class="btn-sm btn-error outline btn-delete">
      ...
      <div ${hasBeenPosted ? '' : raw(`title="Click to edit post content" hx-get="/post/edit/${content.postid}" hx-trigger="click once" hx-target="#post${content.postid}" hx-swap="innerHTML show:#editPost${content.postid}:bottom"`)}>
