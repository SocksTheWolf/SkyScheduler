...
function TrashCanIcon() {
  return (<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" width="20px" height="20px">
...
export function ScheduledPost(props: ScheduledPostOptions) {
  ...
  <article id="postBase${content.postid}" ${oobSwapStr}>
    <div id="post${content.postid}">
      ${hasBeenPosted ? '' : raw(`<button type="submit" hx-delete="/post/delete/${content.postid}" 
        hx-confirm="Are you sure you want to delete this post?" hx-target="#postBase${content.postid}" 
        hx-swap="outerHTML" hx-trigger="click" class="btn-sm btn-error outline btn-delete">
        ${<TrashCanIcon />}
      </button>`)}
      <div ${hasBeenPosted ? '' : raw(`title="Click to edit post content" hx-get="/post/edit/${content.postid}" 
