...
export function ScheduledPost(content: Post) {
  ...
  <article>
    <div id="post${postID}">
      ...
      <div ${hasBeenPosted ? '' : raw(`title="Click to edit post content" hx-get="/post/edit/${content.postid}" hx-trigger="click once" hx-target="#post${postID}" hx-swap="innerHTML"`)}>
    ...
    <footer>
      <small>
        ${hasBeenPosted ? 
          raw(`<a class="secondary has-posted" data-uri="${content.uri}" target="_blank" title="link to post">Posted on</a>:`) : 
          'Scheduled for:' } 
