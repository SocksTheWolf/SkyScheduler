...
export function ScheduledPost(content: Post) {
  ...
  <div class="border border-gray-300 rounded px-3 py-2 bg-white dark:bg-gray-500">
    <div class="flex gap-2 items-center">
      <p class="text-gray-800 flex-1 truncate">${content.text}</p>
    ...
    <p class="text-sm text-gray-500 dark:text-gray-900 mt-1">${content.posted ? 'Posted on:' : 'Scheduled for:'} <span class="timestamp">${content.scheduledDate}</span>${content.repostCount! ? ' | Reposts Left: ' + content.repostCount : ''}</p>
  </div>`;
};
