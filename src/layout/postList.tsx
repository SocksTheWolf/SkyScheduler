...
export function ScheduledPost(content: Post) {
  return html`
  <div class="shadow-2xl card glow border border-gray-300 rounded px-3 py-2 bg-white dark:bg-gray-900/50 dark:text-white">
    <div class="flex gap-2 items-center">
      <p class="text-gray-800 dark:text-white flex-1 truncate">${content.text}</p>
      ${content.posted ? '' : raw(`<button type="submit" hx-post="/post/${content.postid}/delete" hx-target="#posts" hx-swap="innerHTML" hx-trigger="click" class="btn btn-sm btn-error btn-outline px-1 py-0.5">
    ...
    <p class="text-sm text-gray-500 dark:text-white-100 mt-1">${content.posted ? 'Posted on:' : 'Scheduled for:'} <span class="timestamp">${content.scheduledDate}</span>${content.repostCount! ? ' | Reposts Left: ' + content.repostCount : ''}</p>
  </div>`;
};
